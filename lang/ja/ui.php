<?php

return [
    'home' => 'ホーム',
    'play' => 'プレイ',
    'boards' => '自分のボード',
    'profile' => 'プロフィール',
    'admin' => '管理画面',
    'language' => '言語',
    'menu' => 'メニュー',
    'close' => '閉じる',
    'confirm' => '確認',
    'cancel' => 'キャンセル',
    'save' => '保存',
    'delete' => '削除',
    'edit' => '編集',
    'create' => '作成',
    'back' => '戻る',
    'next' => '次へ',
    'previous' => '前へ',
    'submit' => '送信',
    'loading' => '読み込み中…',
    'site_name' => 'ピロープレイ',
    'tagline' => 'オンラインカップルゲームプラットフォーム',
    'switch_language' => '言語を切り替える',
    'copy' => 'コピー',
    'copied' => 'コピーしました',
    'copied_excl' => 'コピーしました！',

    // Profile page
    'username_label' => 'ユーザー名',
    'member_status' => '会員ステータス',
    'free_member' => '無料会員',
    'expires_at_short' => '有効期限：:date',
    'upgrade_premium' => 'プレミアムにアップグレード →',
    'register_date' => '登録日',
    'my_boards' => '自分のボード',
    'new_board' => '新しいボード',
    'square_count' => ':n マス',
    'share_code_tip' => 'シェアコード（クリックでコピー）',
    'play' => '遊ぶ',
    'edit' => '編集',
    'confirm_delete' => '削除しますか？',
    'no_boards_html' => 'まだボードがありません。:link',
    'create_one_now' => '今すぐ作る',
    'custom_games' => 'カスタムゲーム',
    'custom_dice' => 'カスタムサイコロ',
    'custom_wheel' => 'カスタムルーレット',
    'custom_dare' => 'カスタム挑戦',
    'coming_soon' => '近日公開',
    'ad_label' => '広告',
    'ad_remove' => '広告なしで遊びたい？Premium で全ページ広告なし',
    'scroll_more' => '下にもあります — カスタムルーレット',

    'play_history' => 'プレイ履歴',
    'no_play_history' => 'まだプレイ履歴がありません。遊びに行きましょう！',
    'history_with' => ':names と一緒に',
    'history_alone' => 'ひとりで',
    'history_duration' => ':minutes 分プレイ',
    'history_ongoing' => '進行中',
    'my_dice_desc' => '自分だけのサイコロを作る',
    'my_wheels_desc' => '自分だけのルーレットを作る',
    'my_dice' => 'マイダイス',
    'my_wheels' => 'マイルーレット',
    'history_as_host' => 'ホスト',
    'history_total' => '全 :total 件',
    'history_locked' => '無料会員は直近 :count 件のみ表示されます。あと :hidden 件はアップグレードすると閲覧できます。',
    'history_upgrade_cta' => 'アップグレードして全履歴を見る',
    'community_boards' => 'コミュニティボード',
    'theme_rose' => 'ローズ',
    'theme_indigo' => 'インディゴ',
    'theme_switch' => 'テーマ切替',
    'rights_reserved' => '掲載中のお題および文章はすべてオリジナルです。無断転載を禁じます。',

    // ベータ版のお知らせ(PC は右下のカード、スマホは下部の細いバー)— config/beta.php 参照
    'beta_badge' => 'ベータ版',
    'beta_title' => '現在ベータ版です',
    'beta_desc' => 'お題や機能はまだ調整中で、内容が予告なく更新・リセットされることがあります。これからさらに良くなっていきます。',
    'beta_ok' => 'わかりました',
    'beta_report' => '問題を報告',
    'beta_close' => 'お知らせを閉じる',
];
