...
@section('title', __('games.fc_room_title', ['code' => $game->code]) . ' — ' . __('ui.site_name'))
@section('meta_description', __('games.fc_room_meta', ['code' => $game->code]))
...
<div class="game-page">
    {{-- Sidebar --}}
    <aside class="game-sidebar" aria-label="{{ __('games.fc_aria_info') }}">
        ...
        <div class="players-box">
            ...
            <ul class="players-list" id="players-list" aria-live="polite">
                ...
                <li class="player-item player-{{ $p->color }}" data-color="{{ $p->color }}">
                    ...
                    @if($p->is_host && !str_starts_with($p->session_id, 'bot_'))<span class="host-badge">{{ __('games.badge_host') }}</span>@endif
                    @if($myPlayer && $p->session_id === $myPlayer->session_id)<span class="me-badge">{{ __('games.badge_me') }}</span>@endif
                </li>
                @endforeach
            </ul>
        </div>
        ...
        @if($game->isWaiting())
                ...
                <form action="{{ route('games.join', $game->code) }}" method="POST" id="join-form">
                    ...
                    <div class="form-group">
                        <input type="text" name="player_name" class="form-control"
                            placeholder="{{ __('games.name_placeholder') }}" maxlength="20" required
            ...
            <div class="host-controls">
                <p class="waiting-text" id="waiting-text">{{ __('games.waiting_players') }}</p>
                <p style="font-size:.82rem;color:var(--text-dim);margin-bottom:8px">
                    {!! __('games.share_room_text', ['code' => '<strong>'.e($game->code).'</strong>']) !!}
                </p>
                ...
                <p class="hint-text">{{ __('games.min_players_hint') }}</p>
                <a href="{{ route('games.lobby') }}" class="btn btn-sm btn-outline" style="margin-top:8px;width:100%;text-align:center">{{ __('games.back_to_lobby') }}</a>
            </div>
            @else
            <div class="waiting-box">
                <p class="waiting-text">{{ __('games.waiting_host_start') }}</p>
                <p style="font-size:.82rem;color:var(--text-dim);margin-bottom:12px">
                    {{ __('games.host_will_start') }}
                </p>
                <div class="spinner" aria-label="{{ __('games.aria_waiting') }}"></div>
                <a href="{{ route('games.lobby') }}" class="btn btn-sm btn-outline" style="margin-top:16px;width:100%;text-align:center">{{ __('games.back_to_lobby') }}</a>
            </div>
            @endif
        @endif
        ...
        <div class="dice-box">
            ...
            <button id="roll-btn" class="btn btn-primary btn-full" onclick="rollDice()" disabled>
                ...
                {{ __('games.roll_dice_btn') }}
            </button>
            <p id="roll-hint" class="hint-text" style="margin-top:6px;text-align:center;font-size:.8rem">{{ __('games.wait_your_turn') }}</p>
            @endif
        </div>

        <div class="pieces-box">
            <h3>{{ __('games.my_pieces') }}</h3>
            <div id="my-pieces" class="my-pieces" aria-live="polite"></div>
        </div>
        @endif

        <div class="log-box">
            <h3>{{ __('games.game_log') }}</h3>
            <ul id="game-log" class="game-log" aria-live="polite" aria-label="{{ __('games.game_log') }}"></ul>
        </div>
    </aside>

    {{-- Board --}}
    <main class="game-main" aria-label="{{ __('games.fc_aria_board') }}">
        @if($game->isWaiting())
            ...
            <div class="winner-card">
                ...
                <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-top:12px">
                    <a href="{{ route('games.lobby') }}" class="btn btn-primary">
                        ...
                        {{ __('games.back_lobby_short') }}
                    </a>
                    <a href="{{ route('home') }}" class="btn btn-outline">{{ __('games.back_home_short') }}</a>
                </div>
            </div>
        </div>
        @endif
